...
class SamParseJob : public BamThreadJob
{
protected:
    SamLine_t & data;
public:
...
BamAlignment * SamReader::LoadNextAlignment()
{
    ...
    while(!finished && jobs.size() < 10) {
        ...
        pool->addJob(parse_job);
        cerr << "push" << endl;
    }

    while(jobs.size() != 0 && !jobs.back()->parsed) {
        usleep(2000);
    }
    cerr << "pop" << endl;
    SamLine_t * s = jobs.back();
    jobs.pop();
    return s->al;
#else
